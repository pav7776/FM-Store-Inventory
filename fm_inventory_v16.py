...
class MainWindow(QMainWindow):
    ...
    def page_reports(self):
        ...
        controls.addWidget(gen, 2, 1); controls.addWidget(ex, 2, 3)
    ...
    def generate_report(self):
        ...
        if typ in ["Balance Stock Report", "Complete Inventory Report", "Department-wise Stock Report", "Category-wise Stock Report", "Supplier-wise Purchase Report", "Store Location-wise Stock Report"]:
            ...
            sql="""SELECT si.inward_no,si.material_id,i.item_description,c.name category,d.name department,s.name supplier,si.quantity,si.unit_cost,si.total_cost,si.procurement_date,si.po_no,si.do_no,si.remarks
                   ...
                   WHERE 1=1"""
            sql, params = self._append_inventory_filter_sql(sql, [])
            rows=self.db.query(sql, tuple(params)); cols=[("Inward No","inward_no"),("Material ID","material_id"),("Item Description","item_description"),("Category","category"),("Department","department"),("Supplier","supplier"),("Qty","quantity"),("Unit Cost (OMR)","unit_cost"),("Total (OMR)","total_cost"),("Date","procurement_date"),("PO No","po_no"),("DO No","do_no"),("Remarks","remarks")]
